...
app.post('/api/trade/offer', async (c) => {
  ...
  { const _sp=(m)=>{ if(!m) return false; const i=Number(m.id); return i===152||i===153||i===154||(i>=935&&i<=939)||i===957||m.uncapturable===true||m.isBoss===true||m.isGymLeader===true||m.isWarBoss===true; }; if(_sp(body.monster)) return jsonError(c, 400, 'cannot_trade_special'); }
...
app.post('/api/trade/complete', async (c) => {
  ...
  { const _sp=(m)=>{ if(!m) return false; const i=Number(m.id); return i===152||i===153||i===154||(i>=935&&i<=939)||i===957||m.uncapturable===true||m.isBoss===true||m.isGymLeader===true||m.isWarBoss===true; }; if(_sp(fromMonster)||_sp(toMonster)) return jsonError(c, 400, 'cannot_trade_special'); }
