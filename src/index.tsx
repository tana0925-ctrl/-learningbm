  ...
  for (const s of subs.results) {
    ...
    try {
      const aiRes: any = await c.env.AI.run('@cf/meta/llama-3.1-8b-instruct', {
        messages: [
          { role: 'system', content: `小学校の先生が家庭学習カードに赤ペンで書く短いコメントを1つだけ出力してください。

書き方のルール:
- 20〜35文字程度。短くてOK
- 子供に直接語りかける口調（「〜だね！」「〜してえらいね！」「〜がんばったね！」）
- 学習内容と時間に触れる
- 嘘は書かない（データが少ない場合は無理に褒めない）
- 過去データがあれば「前より〜」と成長に触れる
- コメントの文だけ出力。説明や前置きは不要

良い例:
- 算数41分もがんばったね！集中力がすごい！
- 前より学習時間がのびてるね！この調子！
- 漢字練習ていねいに書けてるね！
- 毎日コツコツ続けてるのがえらいよ！` },
