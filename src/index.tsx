...
app.get('/api/student/progress', async (c) => {
  ...
  // __DEFSTAGE_BONUS_V1__ ステージ初クリアのボーナス（コイン＋限定キャラ）。台帳の未適用ぶんだけを配る。
  if (_stateJson) {
    try {
...
app.put('/api/student/progress', async (c) => {
  ...
  try {
    ...
    if (_cur?.state_json) {
      ...
      //    （連絡帳コイン _contactCoinsApplied とまったく同じ作法。台帳は progress 内の累計値）
      const _srvMi = Number(_srv._miCoinsApplied) || 0
      const _cliMi = Number(_inc._miCoinsApplied) || 0
      if (_srvMi > _cliMi) {
        _inc.coins = (Number(_inc.coins) || 0) + (_srvMi - _cliMi)
      ...
      // 👾 ステージ初クリアのボーナスコイン：サーバが付与済みなら、古い端末の全置換保存でも必ず補填。
...
// __DEFSTAGE_30_V1__ 敵の強さが 30 で頭打ちなのに合わせて、ボーナスの計算も 30 で頭打ちにする
// （31回目以降のクリアで台帳が増え続けないようにするため）。
const DEFSTAGE_BONUS_COIN_BASE = 30
const DEFSTAGE_BONUS_COIN_PER_STAGE = 10
const DEFSTAGE_BONUS_MAX_STAGE = 30
...
// __DEF_AUTO_1230_V1__ 平日（月〜金）12:30 JST に防衛戦を自動ではじめる。
...
async function defenseSettings(env: any) {
  ...
  return await defAutoAdvanceV1(env, { active: s.defense_active === '1', decisionAt, eventKey: String(s.defense_event_key || decisionAt || '') })
}
...
app.get('/api/defense/status', async (c) => {
  ...
  const out: any = { ok: true, active: st.active, decision_at: st.decisionAt, event_key: st.eventKey, class_id: classId, base_hp: DEFENSE_BASE_HP, enemy_squad: DEFENSE_ENEMIES, my_entry: null, decided: false, result: null, entries: null, my_reward: null, stage: 1 }
  ...
  // DEF_STAGE_V1 __DEFSTAGE_SENTINEL_V1__ 読むだけ。行が無ければ stage=1。status では絶対に書き込まない
  try {
  ...
  // __DEFSTAGE_BONUS_V1__ 直前に勝った回のステージボーナス（この子ぶん）。読むだけ。ここでは絶対に書かない。
...
app.get('/api/teacher/student-full-analysis', async (c) => {
  ...
  const [submissions, subjectResults, plans, reflections, revisions] = await Promise.all([
    ...
    c.env.DB.prepare(`
      SELECT week_key, plans_json, revision_count, plan_approved, plan_reward_coins,
             reflection_comment, reflection_returned_at, reflection_reward_coins, plan_suggestion, plan_suggestion_at
  ...
  // 📘 ドリルパーク（1問ごとの正誤つき）。単元ごとの正答率より具体的な助言の材料になる。
  let drill: any = null
  try {
  ...
  return c.json({
    ...
    plans: allPlans.map((p: any) => ({ weekKey: p.week_key, revisionCount: p.revision_count || 0, approved: !!p.plan_approved, planSuggestion: p.plan_suggestion || '' })),
    ...
    planSuggestion, mi: miInfo, aiComment: aiComment2,
    testScores,
    records,
    teacherNotes,
    drill,
...
app.post('/api/teacher/test-scores/save', async (c) => {
  ...
  for (const it of body.rows) {
    const uid = String((it && it.userId) || '')
    if (!uid || !allowed.has(uid)) continue
    const evalKnowledge = _recNormRank(String((it && it.evalKnowledge) || ''))
...
//   ここで使っていた generateFeedback() は、第4段階で auto-feedback ごと削除しました。
...
// 🎟️ 2026-09: この児童のクラスでシール交換けんが使えるか。
...
app.get('/api/shop/sticker/current', async (c) => {
  ...
  const nowIso = new Date().toISOString()
  if (!v) return c.json({ ok: true, enabled: stickerOn, boughtToday: false, serverTime: nowIso, voucher: null })
  const now = Date.now()
  ...
  return c.json({ ok: true, enabled: stickerOn, boughtToday: true, serverTime: nowIso, voucher: { id: v.id, status, expiresAt: v.expires_at, remainingMs, redeemedAt: v.redeemed_at || null } })
})
...
// 🚨 2026-09-03 緊急対応: ここが D1 の読み取りを食い尽くしていた。
...
// KAHOOT_TICKET_V1 🎫 カフート券のショップUIを配る道。student-karte.js とまったく同じ形。
...
app.get('/defense2.js', async (c) => { try { const a = await c.env.ASSETS?.fetch(new Request(new URL('https://assets/defense2.js'))); if (a && a.status === 200) return new Response(await a.text(), { headers: { 'content-type': 'application/javascript; charset=utf-8', 'cache-control': 'public, max-age=300' } }); } catch (e) {} return c.text('not found', 404) });const _g8files = ['g8core.js','g8math.js','g8eng.js','g8sci.js','g8soc.js','g8jp.js','g8wild.js','g9core.js','g9math.js','g9eng.js','g9sci.js','g9soc.js','g9jp.js','g9wild.js','g10core.js','g10math.js','g10sci.js','g10soc.js','g10wild.js','hanshin_advice2.js']
...
app.get('/', async (c) => {
  try {
    if (!_rootHtmlCache) {
      ...
      //    判定側がすでに使っている trainingQ.isDecimal に、入力側も合わせる。
      t = t.replace("            const enabled = (trainingMode === 'decimal');", "            const enabled = (trainingMode === 'decimal') || !!(window.trainingQ && window.trainingQ.isDecimal);")
      t = t.replace("                if (trainingMode !== 'decimal') return;", "                if (trainingMode !== 'decimal' && !(window.trainingQ && window.trainingQ.isDecimal)) return;")
      // 問題を作った直後にボタンの有効/無効を計算し直す（単元選択時だけだと、その時点ではまだ問題が無い）
      t = t.replace("    window.trainingQ = { q: prob.q, ans: prob.ans, isDecimal: isDecimalAns };", "    window.trainingQ = { q: prob.q, ans: prob.ans, isDecimal: isDecimalAns };\n    try { if (typeof window.toggleDecimalButton === 'function') window.toggleDecimalButton(); } catch (_e) {}")
      // iOS のテンキーは inputmode="numeric" だと小数点が出ない。答え欄は decimal にする（pattern は入力を弾くので外す）
      t = t.replace(`id="reviewModalInput" inputmode="numeric"`, `id="reviewModalInput" inputmode="decimal"`)
      t = t.replace(`inputmode="numeric" pattern="[0-9]*" placeholder="答え"`, `inputmode="decimal" placeholder="答え"`)
      // 🔥 九九の単元IDが m2-kuku に改名されたのに報酬側が旧IDのままで、いちばん簡単な単元だけ報酬が5〜8倍になっていた
      t = t.replace(`                else if (trainingMode === 'kuku') labelEl.innerText = "10コインGET確率";`, `                else if (trainingMode === 'kuku' || trainingMode === 'm2-kuku') labelEl.innerText = "10コインGET確率";`)
      t = t.replace("            if (trainingMode === 'kuku') gainedExp = 1;", "            if (trainingMode === 'kuku' || trainingMode === 'm2-kuku') gainedExp = 1;")
      t = t.replace("                } else if (trainingMode === 'kuku' || trainingMode === 'conjunction') {", "                } else if (trainingMode === 'kuku' || trainingMode === 'm2-kuku' || trainingMode === 'conjunction') {")
      t = t.replace("                    } else if (trainingMode === 'kuku') {", "                    } else if (trainingMode === 'kuku' || trainingMode === 'm2-kuku') {")
      ...
      //    小6理科3単元(genPlant6 ほか)と同じ作法でここに足す。__R5_MEDAKA_HUMAN__
      t = t.replace("function genFlowWater5(){return _pickBank(_SB5ext.fw5);}", "function genFlowWater5(){return _pickBank(_SB5ext.fw5);}function genMedaka5(){return _pickBank([{q:'メダカのおすとめすを見分けるとき、注目するひれはどれ？',correct:'せびれとしりびれ',wrongs:['おびれとむなびれ','はらびれとおびれ','むなびれとせびれ']},{q:'受精卵とは、どのようなものですか？',correct:'めすがうんだ卵とおすの精子が結びついたもの',wrongs:['めすがうんだばかりで精子と結びつく前の卵','おすが出した精子だけがたくさん集まったもの','卵からかえったばかりの小さな子メダカ']},{q:'メダカのたまごは、かえるまでにおよそどれくらいかかりますか？',correct:'約2週間',wrongs:['約2日','約2か月','約半年']},{q:'たまごの中のようすを大きくして観察するときに使う道具は？',correct:'かいぼうけんび鏡',wrongs:['ほう位じしん','メスシリンダー','記録温度計']},{q:'たまごからかえったばかりの子メダカが、2〜3日えさを食べなくてもよいのはなぜ？',correct:'はらのふくろに養分が入っているから',wrongs:['水の中の養分を体から吸うから','親メダカが口うつしで養分をやるから','日光を受けて自分で養分を作るから']},{q:'メダカを飼うとき、水そうはどこに置くとよいですか？',correct:'日光が直接当たらない明るいところ',wrongs:['一日中日光が強く当たるところ','光がまったく入らない暗いところ','だんぼうのふき出し口のすぐ近く']}]);}try{window.genMedaka5=genMedaka5;}catch(e){}function genHuman5(){return _pickBank([{q:'女性の卵と男性の精子が結びつくことを何といいますか？',correct:'受精',wrongs:['発芽','成長','分裂']},{q:'受精卵が育っていく、母親の体の中の場所はどこ？',correct:'子宮',wrongs:['たいばん','へそのお','ようすい']},{q:'母親からの養分や酸素が子どもに送られる通り道は？',correct:'へそのお',wrongs:['ようすい','子宮のかべ','はいのくだ']},{q:'子宮の中で子どもをつつみ、外からのしょうげきをやわらげているものは？',correct:'ようすい',wrongs:['へそのお','たいばん','子宮口']},{q:'人の子どもが母親の子宮の中で育つ期間は、およそどれくらい？',correct:'約38週',wrongs:['約20週','約50週','約8週']},{q:'生まれたばかりの赤ちゃんの体重は、およそどれくらい？',correct:'約3000g',wrongs:['約300g','約1000g','約6000g']}]);}try{window.genHuman5=genHuman5;}catch(e){}")
      ...
      // HS_NEXT_RECALL_V1_WIRED 入力の すぐ上に まえの「つぎにすること」を 出すだけ。中身は public/hs_next_recall.js。
      t = t.replace('</body>', '<script src="/hs_next_recall.js?v=1"></script></body>')
      // ===== hs-photo-submit-20260911 =====
      // P1 旧ブロック撤去（ID重複の解消）
      t = t.replace(/<div id="hsPhotoSection"[\s\S]*?<div id="hsPhotoAnalysis"[\s\S]*?<\/div>\s*<\/div>/, "<!-- 2026-09: 旧・単一写真ブロックを撤去。hsPhotoSection / hsPhotoInput / hsPhotoStatus のID重複を解消 -->")
      // P2 提出前にボーナスの存在を知らせる
      t = t.replace("<div style=\"font-size:13px; font-weight:bold; color:#0e7490;\">📷 成果物の写真（任意・複数OK）</div>", "<div style=\"font-size:13px; font-weight:bold; color:#0e7490;\">📷 写真をつけるとボーナス！<span style=\"font-weight:normal; font-size:11px;\">（1日1回）</span></div>\n          <div style=\"font-size:11px; color:#0e7490; line-height:1.5; margin:2px 0 4px;\">がんばったノートやプリントの写真をつけると、コインなどがもらえるよ。すごくまれに、とくべつなことが起きるかも。写真なしでも、ふつうに提出できます。</div>")
      // P3 multiple を外す（1日1枚）
      t = t.replace("写真を追加\n            <input type=\"file\" id=\"hsPhotoInput\" accept=\"image/*\" multiple style=\"display:none;\" onchange=\"hsPhotosSelected(this)\"/>", "写真をえらぶ\n            <input type=\"file\" id=\"hsPhotoInput\" accept=\"image/*\" style=\"display:none;\" onchange=\"hsPhotosSelected(this)\"/>")
      // P4 枚数表示の是正＋送信前の縮小（長辺1200px/JPEG0.7）
      t = t.replace("    if (_hsPhotoFiles.length >= 5) { alert('写真は最大5枚までです'); break; }\n    _hsPhotoFiles.push(file);\n    hsAddPhotoPreview(file, _hsPhotoFiles.length - 1);\n  }\n  input.value = '';\n  if (statusEl) statusEl.textContent = _hsPhotoFiles.length ? '📎 ' + _hsPhotoFiles.length + '枚セット済み' : '';\n  // 最初の1枚だけAI分析（教師用）\n  if (_hsPhotoFiles.length > 0 && !_hsPhotoAnalysisResult) {\n    var sess = hsGetSession();\n    var dayKey = sess ? sess.dayKey : hsGetDayKey830(new Date());\n    var fd = new FormData();\n    fd.append('photo', _hsPhotoFiles[0]);\n    fd.append('dayKey', dayKey);\n    fetch('/api/homework/analyze-photo', { method: 'POST', body: fd })", "    if (_hsPhotoFiles.length >= 1) { _hsPhotoFiles = []; _hsPhotoAnalysisResult = ''; var _pl0 = document.getElementById('hsPhotoPreviewList'); if (_pl0) _pl0.innerHTML = ''; }\n    _hsPhotoFiles.push(file);\n    hsAddPhotoPreview(file, _hsPhotoFiles.length - 1);\n    break;\n  }\n  // 送信前に写真を小さくする（長辺1200px / JPEG 0.7）。うまくいかないときは元のまま送る。\n  function hsShrinkPhoto(f, cb){\n    try{\n      if (!f || !/^image\\//.test(f.type || '')) { cb(f); return; }\n      var u = URL.createObjectURL(f);\n      var im = new Image();\n      im.onload = function(){\n        try{\n          var MX = 1200;\n          var w = im.naturalWidth || im.width, h = im.naturalHeight || im.height;\n          if (!w || !h) { URL.revokeObjectURL(u); cb(f); return; }\n          var sc = Math.min(1, MX / Math.max(w, h));\n          var cw = Math.max(1, Math.round(w * sc)), ch = Math.max(1, Math.round(h * sc));\n          var cv = document.createElement('canvas'); cv.width = cw; cv.height = ch;\n          var cx = cv.getContext('2d');\n          cx.imageSmoothingEnabled = true;\n          try { cx.imageSmoothingQuality = 'high'; } catch(_q){}\n          cx.drawImage(im, 0, 0, cw, ch);\n          cv.toBlob(function(b){ URL.revokeObjectURL(u); cb(b && b.size && b.size < f.size ? b : f); }, 'image/jpeg', 0.7);\n        }catch(_a){ URL.revokeObjectURL(u); cb(f); }\n      };\n      im.onerror = function(){ URL.revokeObjectURL(u); cb(f); };\n      im.src = u;\n    }catch(_b){ cb(f); }\n  }\n  input.value = '';\n  if (statusEl) statusEl.textContent = _hsPhotoFiles.length ? '📎 写真をセットしたよ' : '';\n  // 最初の1枚だけAI分析（教師用）\n  if (_hsPhotoFiles.length > 0 && !_hsPhotoAnalysisResult) {\n    var sess = hsGetSession();\n    var dayKey = sess ? sess.dayKey : hsGetDayKey830(new Date());\n    hsShrinkPhoto(_hsPhotoFiles[0], function(_sent){\n    var fd = new FormData();\n    fd.append('photo', _sent, 'photo.jpg');\n    fd.append('dayKey', dayKey);\n    fetch('/api/homework/analyze-photo', { method: 'POST', body: fd })")
      ...
      // __DEF_LV50_V1__ ぼうえいせんの ものさしを そろえる（レベル50・星と つかれ なし・上限つき）と、その しるし
      t = t.replace("function _defSnapshot(id){ try{ var base=getMonster(Number(id)); if(!base) return null; var inst=(player.monsters&&(player.monsters[id]||player.monsters[String(id)]))||{level:1}; var lvl=Math.max(1,Number(inst.level||1)); var s=getStats(base,lvl);", "/*__DEF_LV50_V1__ ぼうえいせんの ものさし。レベル50・星と つかれは なし・atk/def/spd は 300 まで・合計 5000 まで。しゅるいごとの つよさは のこる*/window.defNorm=function(b){var L=50,sm=Number((b&&b.stage)||1),id=Number(b&&b.id),s;if(id===152){s={hp:5000,atk:300,def:300,spd:300};}else if(id===153){s={hp:8000,atk:500,def:500,spd:500};}else if(id===154){s={hp:9999,atk:777,def:777,spd:777};}else if(id===999){s={hp:Math.floor(8500+35*(L-1)),atk:Math.floor(Number(b.atk||10)+L*1.5+L*sm*0.5),def:Math.floor(Number(b.def||5)+L*1.2+L*sm*0.4),spd:Math.floor(Number(b.spd||5)+L*1.1+L*sm*0.3)};}else{s={hp:Math.floor(Number(b.hp||100)*3+L*30+L*sm*5),atk:Math.floor(Number(b.atk||10)+L*1.5+L*sm*0.5),def:Math.floor(Number(b.def||5)+L*1.2+L*sm*0.4),spd:Math.floor(Number(b.spd||5)+L*1.2+L*sm*0.4)};}s.atk=Math.min(s.atk,300);s.def=Math.min(s.def,300);s.spd=Math.min(s.spd,300);var tot=s.hp+s.atk+s.def+s.spd;if(tot>5000){var k=5000/tot;s={hp:Math.floor(s.hp*k),atk:Math.floor(s.atk*k),def:Math.floor(s.def*k),spd:Math.floor(s.spd*k)};}s.hp=Math.max(1,s.hp);s.atk=Math.max(1,s.atk);s.def=Math.max(1,s.def);s.spd=Math.max(1,s.spd);s.maxHp=s.hp;return s;};window._defSnapshot=function(id){return _defSnapshot(id);};function _defSnapshot(id){ try{ var base=getMonster(Number(id)); if(!base) return null; var inst=(player.monsters&&(player.monsters[id]||player.monsters[String(id)]))||{level:1}; var lvl=50; var s=window.defNorm(base);")
      t = t.replace("return {id:Number(id),name:base.name,sprite:base.sprite,level:lvl,", "return {id:Number(id),name:base.name,sprite:base.sprite,level:lvl,dn:1,")
      // DEF2_LV50_SAME_V1 DEF2_LV50_ENTRYFIX_V1 出陣の 画面で、みんな おなじ ものさしだと つたえる（見出しが ステージ表示に 書きかえられた あとでも あたるよう、あて先を みじかくした）
      t = t.replace(`var enemyHtml='<div style="font-weight:900;margin:6px 0;">👾 `, `var enemyHtml='<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:8px 10px;margin-bottom:8px;color:#1e40af;font-size:12px;font-weight:700;line-height:1.6;">みんな おなじ レベル50 で たたかうよ。かちまけは プログラムの くふうで きまるんだ。</div><div style="font-weight:900;margin:6px 0;">👾 `)
    ...
    if (d.carry_over_error==='monster_gone') {`)
      // __H6MERGE_SRC_V1__ 小6歴史 _SC(死蔵バンク) を _SS に結合し、範囲外・重複を外して不足分を足す
      t = t.replace("function genH6u1(){return _pickBank(_SS.h6u1);}\nfunction genH6u2(){return _pickBank(_SS.h6u2);}\nfunction genH6u3(){return _pickBank(_SS.h6u3);}\nfunction genH6u4(){return _pickBank(_SS.h6u4);}\nfunction genH6u5(){return _pickBank(_SS.h6u5);}\nfunction genH6u6(){return _pickBank(_SS.h6u6);}\nfunction genH6u7(){return _pickBank(_SS.h6u7);}\nfunction genH6u8(){return _pickBank(_SS.h6u8);}\nfunction genH6u9(){return _pickBank(_SS.h6u9);}\nfunction genH6u10(){return _pickBank(_SS.h6u10);}\nfunction genH6u11(){return _pickBank(_SS.h6u11);}", "/*__H6BANK_LIVE_V1__*/var _H6DROP={\"h6u1\":[\"縄文時代の人が住んでいた家を何という\",\"縄文時代の土器の特ちょうは\",\"弥生土器の特ちょうは\",\"古墳時代に作られた土の人形を何という\",\"縄文時代の食べ物として正しいものは\",\"中国の歴史書に倭奴国王と記された王が送った使いは\",\"卑弥呼が呪術で国をおさめたとは\",\"弥生時代の農具として使われたものは\",\"縄文時代に使われた矢のはずしは\",\"邪馬台国の邪馬台の読みは\"],\"h6u2\":[\"聖徳太子が中国隋に送った使いを何という\",\"奈良時代に編集された日本最古の歴史書は\",\"聖徳太子が建てたとされる最古の仏教寺院は\",\"聖徳太子が遣隋使として送った人物は\"],\"h6u3\":[\"平安時代に最も権力を持った貴族の家柄は\",\"平安京が現在のどこにあたる\",\"枕草子の書き出し春はあけぼのを書いたのは\",\"平安時代の貴族が着た正式な服装は\"],\"h6u4\":[\"北条氏が担った役職は何\",\"元モンゴルが2度日本に攻めてきた出来事を何という\",\"元寇で日本を助けた自然現象は\",\"鎌倉幕府が続いたのは何世紀ごろ\",\"鎌倉幕府の成立年として有力な説は\",\"元寇で神風暴風雨が吹いたのはいつ\",\"元寇の後幕府が衰退した最大の理由は\",\"鎌倉時代の問注所もんちゅうじょとは\",\"鎌倉時代の地方における惣領制そうりょうせいとは\",\"鎌倉幕府では将軍と御家人の関係でお礼として与えるものは\",\"御家人が将軍へのお返しとして行うことは\",\"鎌倉時代の侍所さむらいどころとは\",\"吾妻鏡あずまかがみとは\"],\"h6u5\":[\"室町幕府を開いた人物は\",\"室町幕府で将軍の補佐をした役職は\",\"室町幕府が置かれた都市は\",\"室町時代に村人が自治を行った集まりを何という\",\"室町時代に発達した商人・職人の同業者組合は\",\"今もある生け花の流派で最も古いとされるのは\",\"室町時代の農民が年貢の軽減を求めて起こした争いは\",\"応仁の乱で東軍の中心人物は\",\"応仁の乱で西軍の中心人物は\",\"室町幕府の将軍が住んだ場所は\",\"能のせりふと動きを担当するのは\",\"水墨画を大成した禅僧の画家は\",\"戦国時代に各地の戦国大名が制定した法律を何という\",\"足利義政の妻で政治に口を出した女性は\",\"銀閣寺の正式名称は\",\"室町時代の文化の特徴を一言で言うと\",\"金閣の建っている金閣寺鹿苑寺は何宗のお寺\"],\"h6u6\":[\"豊臣秀吉が行った全国の土地の調査は\",\"戦国時代の武将の連合軍を率いて信長を倒したのは\",\"豊臣秀吉が全国の農民の武器を取り上げた命令を何という\",\"信長が使った鉄砲を大量に活用した戦いはどれ\",\"信長が仏教勢力を攻撃した理由は\",\"秀吉が行った身分統制令の内容は\",\"戦国時代の武将で義の将軍と呼ばれたのは\",\"織田信長がついに討ち果たした一向一揆の拠点は\",\"秀吉の出身は\",\"秀吉が1590年に小田原の北条氏を滅ぼして成し遂げたのは\",\"秀吉の検地が農民に与えた影響は\",\"信長の行った楽市・楽座の意味は\",\"秀吉が行った検地・刀狩により確立されたものは\",\"信長が比叡山延暦寺を焼き打ちにした年は\",\"秀吉の政権で活躍した5人の重要な家臣集団を何という\",\"信長・秀吉・家康の三人を表す有名な鳴かぬならの句で信長に当たるのは\",\"秀吉がキリスト教宣教師を追放しようとした命令を何という\"],\"h6u7\":[\"鎖国中も貿易を許された国は\",\"大名を統制した法律を何という\",\"江戸幕府の初代将軍は\",\"鎖国の目的として正しくないのは\",\"関ヶ原の戦いが起きた年は\",\"参勤交代とはどういう制度\",\"鎖国中も外国との窓口だった場所は\",\"鎖国中に交流を続けた国は\",\"大名を監視するために置いた幕府の役人は\",\"幕府が大名を統制するために定めた法律は\",\"江戸幕府で将軍の補佐をした最高位の役職は\",\"幕府が朝廷を統制した法律は\",\"江戸幕府の三代将軍は\",\"徳川家康が征夷大将軍になった意義は\",\"江戸時代の身分制度士農工商で商人より下に置かれた人は\",\"江戸幕府の農民政策田畑永代売買禁止令の目的は\",\"江戸時代に幕府が貿易を管理した場所は\"],\"h6u8\":[\"おくのほそ道を書いた俳人は\",\"江戸時代中期に8代将軍が行った政治改革は\",\"伊能忠敬が作ったものは\",\"江戸時代後期の文化を代表する浮世絵師は\",\"学問のすゝめを書いた明治の学者は誰かではなく江戸期の学問で有名なのは\",\"江戸時代に民間で開かれた教育の場は\",\"江戸時代の浮世絵で有名な富嶽三十六景を描いたのは\",\"東海道五十三次を描いた浮世絵師は\",\"江戸時代の本の普及を支えた業者は\",\"江戸時代後期に民間で流行った教訓物語を何という\",\"忠臣蔵の元になった実際の事件は\",\"江戸時代に武士が学んだ学校は\",\"奥の細道を書いたのは誰\",\"杉田玄白らが翻訳した解体新書は何年に出版\",\"江戸時代に全国的に広まった読み書きそろばんを教えた場所は\",\"東洲斎写楽が描いた浮世絵の題材は\",\"シーボルトが長崎に開いた学校は\",\"伊能忠敬が全国測量を始めた年齢は\",\"仮名手本忠臣蔵を人形浄瑠璃にした作家は\",\"江戸時代後期に幕府の学問所で正学とされた学問は\",\"江戸時代のお笑い芸能で二人が掛け合う芸は\",\"天保の改革で庶民の娯楽を厳しく取り締まった老中は\"],\"h6u9\":[\"文明開化とは\",\"脱亜入欧という言葉を広めた人物は\",\"明治維新が始まった年は\",\"版籍奉還とはどういう改革\",\"廃藩置県で藩の代わりに置かれたものは\",\"明治政府が出した五か条のご誓文の内容は\",\"明治時代に行われた地租改正の内容は\",\"富国強兵の富国とは\",\"大日本帝国憲法が発布された年は\",\"伊藤博文が参考にして憲法を作った国は\",\"日露戦争で有名な日本海海戦を指揮した海軍大将は\",\"明治時代に全国に広まった小学校教育を定めた法律は\",\"不平等条約の改正を成し遂げた外務大臣は\",\"坂の上の雲に描かれた時代は\",\"明治時代の陸軍を整備した長州出身の元勲は\",\"明治時代に廃止された身分制度で武士の特権は\"],\"h6u10\":[\"太平洋戦争で日本が最初に攻撃した場所は\",\"戦時中子どもたちが農村に送られた出来事は\",\"大正時代に起きた民主主義運動を何という\",\"1931年に日本軍が中国東北部満州で起こした事件は\",\"日中戦争が始まった年は\",\"原爆が投下された二つの都市は\",\"広島に原爆が投下された日は\",\"日本軍が南京で行った事件は\",\"戦時中に小学生が集団で農村に移送されたことを何という\",\"学徒出陣とは\",\"戦時中に食料や物資が不足し政府が配る制度は\",\"大正時代に起きた大規模な労働運動の背景は\",\"一九三二年の軍部による政府への反乱事件は\",\"一九三六年の軍部によるクーデターは\",\"ヒットラーが率いたドイツの政党は\",\"第二次世界大戦で連合国側の国は\",\"日本の降伏を受けたアメリカの大統領は\",\"太平洋戦争中日本で行われた玉砕の意味は\",\"長崎に原爆が投下された日は\",\"日本が終戦を迎えた1945年当時の首相は\",\"東条英機が首相だった時代に始まった戦争は\"],\"h6u11\":[]};var _H6ADD={\"h6u1\":[{\"q\":\"🌾 弥生時代に稲の穂を\\nかりとるのに使った道具は？\",\"correct\":\"石包丁（いしぼうちょう）\",\"wrongs\":[\"田げた（たげた）\",\"千歯こき\",\"石おの\"]},{\"q\":\"🌾 稲作はどこから\\n日本に伝わった？\",\"correct\":\"中国や朝鮮半島から九州北部へ\",\"wrongs\":[\"ロシアから北海道へ\",\"南のインドから沖縄へ\",\"ヨーロッパから船で九州へ\"]},{\"q\":\"👑 卑弥呼が中国（魏）の\\n皇帝から送られたものは？\",\"correct\":\"銅鏡（どうきょう）など\",\"wrongs\":[\"仏像とお経\",\"鉄砲と火薬\",\"紙と筆\"]},{\"q\":\"🗿 大和朝廷（やまとちょうてい）の\\n中心があった地域は？\",\"correct\":\"奈良県を中心とした地域\",\"wrongs\":[\"福岡県を中心とした地域\",\"愛知県を中心とした地域\",\"東京都を中心とした地域\"]},{\"q\":\"🗿 5世紀ごろ大和朝廷の\\n王は何と呼ばれた？\",\"correct\":\"大王（おおきみ）\",\"wrongs\":[\"天皇\",\"将軍\",\"太政大臣\"]},{\"q\":\"🏺 縄文時代のたて穴住居の\\n真ん中にあったものは？\",\"correct\":\"火をたくいろり\",\"wrongs\":[\"米をたくわえるかめ\",\"神をまつる祭だん\",\"水をくむ井戸\"]}],\"h6u2\":[],\"h6u3\":[],\"h6u4\":[{\"q\":\"⚔️ 平氏がほろびた\\n最後の戦いは？\",\"correct\":\"壇ノ浦の戦い（だんのうら）\",\"wrongs\":[\"桶狭間の戦い\",\"関ヶ原の戦い\",\"白村江の戦い\"]},{\"q\":\"⚔️ 平清盛が武士として\\n初めてついた朝廷の最高の位は？\",\"correct\":\"太政大臣（だじょうだいじん）\",\"wrongs\":[\"征夷大将軍\",\"執権\",\"関白\"]},{\"q\":\"⚔️ 源頼朝が弟の源義経らと\\n戦った相手は？\",\"correct\":\"平氏（へいし）\",\"wrongs\":[\"北条氏\",\"元（モンゴル）\",\"藤原氏の貴族\"]},{\"q\":\"⚔️ 元軍が使って\\n日本の武士を苦しめた武器は？\",\"correct\":\"火薬を使った「てつはう」\",\"wrongs\":[\"種子島に伝わった鉄砲\",\"大砲\",\"弓矢だけ\"]},{\"q\":\"⚔️ 元軍とふたたび戦うため\\n幕府が博多湾に築いたものは？\",\"correct\":\"石の防塁（ぼうるい）\",\"wrongs\":[\"天守閣のある城\",\"運河\",\"関所\"]},{\"q\":\"⚔️ 鎌倉時代の武士の館（やかた）の\\n特ちょうは？\",\"correct\":\"まわりにほりや塀をめぐらせた\",\"wrongs\":[\"庭に大きな池があり寝殿造りだった\",\"石垣の上に天守閣があった\",\"高い床の倉庫だった\"]},{\"q\":\"⚔️ 「平家物語」を各地で\\n語り伝えた人々は？\",\"correct\":\"琵琶法師（びわほうし）\",\"wrongs\":[\"能楽師\",\"僧兵\",\"歌人\"]},{\"q\":\"⚔️ 源頼朝が鎌倉に幕府を開いたのは\\nどんな土地だったから？\",\"correct\":\"三方を山、一方を海に囲まれ守りやすい土地\",\"wrongs\":[\"米がよくとれる広い平地\",\"天皇が住んでいた都\",\"外国船が入る大きな港\"]},{\"q\":\"⚔️ 承久の乱のあと\\n幕府の力はどうなった？\",\"correct\":\"西日本にも広がって強くなった\",\"wrongs\":[\"弱まって朝廷が政治を行った\",\"ほとんど変わらなかった\",\"幕府はほろびた\"]},{\"q\":\"⚔️ 「御家人（ごけにん）」とは\\nどんな人たち？\",\"correct\":\"将軍と主従関係を結んだ武士\",\"wrongs\":[\"天皇に仕えた貴族\",\"幕府に年貢を納めた農民\",\"寺に仕えた僧\"]},{\"q\":\"⚔️ 元は何度\\n日本に攻めてきた？\",\"correct\":\"2度（文永の役・弘安の役）\",\"wrongs\":[\"1度だけ\",\"3度\",\"5度\"]}],\"h6u5\":[{\"q\":\"🏯 足利義満が金閣を建てた\\n京都の場所は？\",\"correct\":\"北山（きたやま）\",\"wrongs\":[\"東山（ひがしやま）\",\"嵐山（あらしやま）\",\"鞍馬（くらま）\"]},{\"q\":\"🏯 足利義政が銀閣を建てた\\n京都の場所は？\",\"correct\":\"東山（ひがしやま）\",\"wrongs\":[\"北山（きたやま）\",\"宇治（うじ）\",\"伏見（ふしみ）\"]},{\"q\":\"🎭 書院造で生まれ\\n今の和室にも受けつがれたものは？\",\"correct\":\"畳・障子・床の間\",\"wrongs\":[\"いろり・土間・かやぶき屋根\",\"天守閣・石垣・ほり\",\"御簾（みす）・寝殿・池\"]},{\"q\":\"🎭 雪舟が中国（明）に\\nわたって学んできたものは？\",\"correct\":\"水墨画の技法\",\"wrongs\":[\"浮世絵のほり方\",\"大和絵の色づかい\",\"油絵の道具\"]},{\"q\":\"🎭 能を演じるとき\\n主役がつけるものは？\",\"correct\":\"能面（のうめん）\",\"wrongs\":[\"くま取りの化粧\",\"かんむりとかつら\",\"鉄のかぶと\"]},{\"q\":\"🎭 能の合間に演じられ\\n人々を笑わせる劇は？\",\"correct\":\"狂言（きょうげん）\",\"wrongs\":[\"落語\",\"漫才\",\"人形浄瑠璃\"]},{\"q\":\"🏯 室町時代に日本が\\n貿易をしていた中国の王朝は？\",\"correct\":\"明（みん）\",\"wrongs\":[\"唐（とう）\",\"宋（そう）\",\"清（しん）\"]},{\"q\":\"🏯 明との貿易で日本が\\n多く輸入したものは？\",\"correct\":\"銅銭（明のお金）・生糸\",\"wrongs\":[\"鉄砲・火薬\",\"じゃがいも・とうもろこし\",\"石油・石炭\"]},{\"q\":\"🏯 室町時代に各地で力をつけ\\n領地を支配した武士を何という？\",\"correct\":\"守護大名（しゅごだいみょう）\",\"wrongs\":[\"御家人\",\"執権\",\"旗本\"]},{\"q\":\"🏯 応仁の乱で\\n最も大きな被害を受けた都市は？\",\"correct\":\"京都\",\"wrongs\":[\"鎌倉\",\"江戸\",\"大阪\"]},{\"q\":\"🎭 何人かで集まって和歌を\\n上の句・下の句と続けた文芸は？\",\"correct\":\"連歌（れんが）\",\"wrongs\":[\"俳句\",\"川柳\",\"短歌\"]},{\"q\":\"🏯 戦国大名が城のまわりに\\n家来や商人を集めてつくった町は？\",\"correct\":\"城下町（じょうかまち）\",\"wrongs\":[\"門前町\",\"宿場町\",\"港町\"]},{\"q\":\"🎭 雪舟の水墨画の\\n特ちょうは？\",\"correct\":\"墨の濃さやかすれだけで景色を表す\",\"wrongs\":[\"金や銀をたくさん使う\",\"人物を大きく色あざやかにえがく\",\"木の板にほって刷る\"]}],\"h6u6\":[{\"q\":\"🔥 長篠の戦いで織田・徳川の\\n連合軍が破った大名は？\",\"correct\":\"武田氏（武田勝頼）\",\"wrongs\":[\"今川氏\",\"上杉氏\",\"北条氏\"]},{\"q\":\"🔥 鉄砲が伝わったことで\\n城のつくりはどう変わった？\",\"correct\":\"高い石垣や広いほりを持つ城になった\",\"wrongs\":[\"木の柵だけの簡単な城になった\",\"城をつくらなくなった\",\"すべて山の上に移った\"]},{\"q\":\"🌺 太閤検地で\\n調べたことは？\",\"correct\":\"田畑の広さ・土地のよしあし・耕す人\",\"wrongs\":[\"人口と家の数だけ\",\"武士の人数\",\"商人が持つお金の額\"]},{\"q\":\"🌺 刀狩によって\\n武士と農民はどうなった？\",\"correct\":\"身分がはっきり分けられた\",\"wrongs\":[\"農民も武士になれるようになった\",\"武士が農業をするようになった\",\"身分の差がなくなった\"]},{\"q\":\"🌺 豊臣秀吉が朝鮮に\\n軍を送った結果は？\",\"correct\":\"目的を果たせず大名や農民が苦しんだ\",\"wrongs\":[\"朝鮮を支配して領地にした\",\"中国（明）まで攻め取った\",\"貿易が盛んになり豊かになった\"]},{\"q\":\"🔥 安土桃山時代に城の内部を\\nかざった大きな絵を何という？\",\"correct\":\"障壁画（ふすま絵・屏風絵）\",\"wrongs\":[\"浮世絵\",\"水墨画\",\"絵巻物\"]},{\"q\":\"🔥 「唐獅子図屏風」など豪華な\\n屏風絵をえがいた画家は？\",\"correct\":\"狩野永徳（かのうえいとく）\",\"wrongs\":[\"雪舟\",\"葛飾北斎\",\"歌川広重\"]},{\"q\":\"🌺 千利休が大成させた\\n茶の湯の心は？\",\"correct\":\"わび茶（質素で静かな心）\",\"wrongs\":[\"豪華でにぎやかな心\",\"武士が戦にそなえる心\",\"商人が利益を求める心\"]},{\"q\":\"🔥 南蛮貿易で日本に\\nもたらされたものは？\",\"correct\":\"鉄砲・火薬・時計・ガラス製品\",\"wrongs\":[\"絹織物・茶・漢字\",\"自動車・電気\",\"じゃがいも・米\"]},{\"q\":\"🔥 南蛮貿易の\\n相手の国は？\",\"correct\":\"ポルトガルとスペイン\",\"wrongs\":[\"オランダとイギリス\",\"中国と朝鮮\",\"アメリカとロシア\"]},{\"q\":\"🌺 豊臣秀吉が宣教師を\\n追放しようとしたのはなぜ？\",\"correct\":\"信者が増えて国をまとめるのにじゃまだと考えたから\",\"wrongs\":[\"貿易をやめたかったから\",\"仏教を信じていなかったから\",\"外国と戦争をしたかったから\"]},{\"q\":\"🔥 安土城や大阪城が建てられたころの\\n文化を何という？\",\"correct\":\"安土桃山文化\",\"wrongs\":[\"東山文化\",\"元禄文化\",\"天平文化\"]},{\"q\":\"🌺 豊臣秀吉が全国を統一したころ\\n朝廷から与えられた位は？\",\"correct\":\"関白（かんぱく）\",\"wrongs\":[\"征夷大将軍\",\"執権\",\"管領\"]},{\"q\":\"🔥 伝わった鉄砲は\\nどこで多くつくられた？\",\"correct\":\"堺（さかい）や国友（くにとも）\",\"wrongs\":[\"江戸や日光\",\"長崎や平戸\",\"鎌倉や小田原\"]}],\"h6u7\":[{\"q\":\"🏯 1603年、徳川家康が\\n朝廷からついた役職は？\",\"correct\":\"征夷大将軍（せいいたいしょうぐん）\",\"wrongs\":[\"関白\",\"太政大臣\",\"執権\"]},{\"q\":\"🏯 参勤交代で大名が\\n最も苦しんだことは？\",\"correct\":\"行き来や江戸ぐらしの費用が大きな負担になったこと\",\"wrongs\":[\"領地の米がとれなくなること\",\"家来が減ること\",\"城を取りこわされること\"]},{\"q\":\"🏯 幕府が大名を全国に置くとき\\n気をつけたことは？\",\"correct\":\"外様大名を江戸から遠くに置いた\",\"wrongs\":[\"譜代大名を江戸から遠くに置いた\",\"親藩をすべて九州に置いた\",\"すべての大名を江戸に住まわせた\"]},{\"q\":\"🏯 江戸幕府が\\n直接治めた土地を何という？\",\"correct\":\"天領（幕府の直轄地）\",\"wrongs\":[\"荘園\",\"藩\",\"領国\"]},{\"q\":\"🌐 島原・天草一揆で\\n中心となったといわれる少年は？\",\"correct\":\"天草四郎（あまくさしろう）\",\"wrongs\":[\"大塩平八郎\",\"石田三成\",\"由井正雪\"]},{\"q\":\"🌐 絵踏（えぶみ）は\\n何のために行われた？\",\"correct\":\"キリスト教の信者を見つけ出すため\",\"wrongs\":[\"年貢を納めたか調べるため\",\"武士の身分を確かめるため\",\"外国の品物を調べるため\"]},{\"q\":\"🌐 鎖国の間もオランダとの貿易を\\n続けたのはなぜ？\",\"correct\":\"オランダはキリスト教を広めようとしなかったから\",\"wrongs\":[\"オランダが日本にいちばん近かったから\",\"オランダが戦争に強かったから\",\"オランダが年貢を納めたから\"]},{\"q\":\"🌐 鎖国の間に朝鮮から\\n日本へ送られてきた使節は？\",\"correct\":\"朝鮮通信使（ちょうせんつうしんし）\",\"wrongs\":[\"遣唐使\",\"南蛮船\",\"勘合貿易船\"]},{\"q\":\"🌐 鎖国の間、蝦夷地（北海道）で\\nアイヌの人々と交易した藩は？\",\"correct\":\"松前藩（まつまえはん）\",\"wrongs\":[\"対馬藩\",\"薩摩藩\",\"長州藩\"]},{\"q\":\"🏯 五街道の\\n起点とされた場所は？\",\"correct\":\"江戸の日本橋\",\"wrongs\":[\"京都の三条大橋\",\"大阪の淀屋橋\",\"名古屋の熱田\"]},{\"q\":\"🏯 街道に置かれ、通る人や荷物を\\n調べた場所は？\",\"correct\":\"関所（せきしょ）\",\"wrongs\":[\"宿場（しゅくば）\",\"問屋（といや）\",\"飛脚屋\"]},{\"q\":\"🏯 江戸時代、農民が\\n互いに監視し合った仕組みは？\",\"correct\":\"五人組（ごにんぐみ）\",\"wrongs\":[\"惣（そう）\",\"座（ざ）\",\"株仲間\"]},{\"q\":\"🏯 大阪が「天下の台所」と\\n呼ばれたのはなぜ？\",\"correct\":\"全国から米や特産物が集まる商業の中心だったから\",\"wrongs\":[\"幕府が置かれ政治の中心だったから\",\"天皇が住んでいたから\",\"外国との貿易の窓口だったから\"]},{\"q\":\"🏯 武家諸法度が\\n大名に禁じたことは？\",\"correct\":\"勝手に城を修理したり大名どうしで結婚したりすること\",\"wrongs\":[\"参勤交代を行うこと\",\"領地で年貢を集めること\",\"家来を持つこと\"]},{\"q\":\"🌐 鎖国によって\\n日本が受けた影響は？\",\"correct\":\"外国の争いにまきこまれず日本独自の文化が育った\",\"wrongs\":[\"外国の進んだ技術がすぐに入ってきた\",\"全国で戦争が続いた\",\"貿易がまったくなくなった\"]},{\"q\":\"🌐 幕府が鎖国を行った\\n最も大きなねらいは？\",\"correct\":\"キリスト教を防ぎ幕府の力を安定させるため\",\"wrongs\":[\"外国の品物を安く買うため\",\"日本人を外国で働かせるため\",\"新しい領地を手に入れるため\"]}],\"h6u8\":[{\"q\":\"🎪 近松門左衛門が人形浄瑠璃や\\n歌舞伎にえがいたのは？\",\"correct\":\"町人の義理と人情の物語\",\"wrongs\":[\"武士の合戦の物語\",\"貴族の恋の物語\",\"外国での冒険の物語\"]},{\"q\":\"🎪 浮世絵は明治のころ\\n外国でどう受けとめられた？\",\"correct\":\"ヨーロッパの画家たちに大きな影響をあたえた\",\"wrongs\":[\"まったく知られなかった\",\"古くさい絵だとされた\",\"すぐに輸入が禁止された\"]},{\"q\":\"📚 伊能忠敬が正確な地図を\\nつくるために行ったことは？\",\"correct\":\"自分の足で全国を歩いて測量した\",\"wrongs\":[\"船で海の上からながめてえがいた\",\"山の上からながめてえがいた\",\"外国の地図を写した\"]},{\"q\":\"📚 杉田玄白たちが「解体新書」を\\n訳そうと決めたきっかけは？\",\"correct\":\"実際の人の体とオランダの本の図が同じだと確かめたから\",\"wrongs\":[\"幕府に命じられたから\",\"医者の試験に必要だったから\",\"中国の本と同じ内容だったから\"]},{\"q\":\"📚 寺子屋で町人や農民の子は\\n何を学んだ？\",\"correct\":\"読み・書き・そろばん\",\"wrongs\":[\"剣術と馬術\",\"漢文と兵法\",\"蘭学と医学\"]},{\"q\":\"📚 藩校で武士の子が\\n中心に学んだ学問は？\",\"correct\":\"儒学（じゅがく）\",\"wrongs\":[\"蘭学\",\"国学\",\"測量術\"]},{\"q\":\"🎪 葛飾北斎の「富嶽三十六景」に\\nくり返しえがかれたものは？\",\"correct\":\"富士山\",\"wrongs\":[\"江戸城\",\"東海道の宿場\",\"歌舞伎役者\"]},{\"q\":\"🎪 歌川広重の「東海道五十三次」が\\nえがいたものは？\",\"correct\":\"江戸から京都までの宿場の風景\",\"wrongs\":[\"江戸の町の祭り\",\"大阪の商店のようす\",\"富士山だけ\"]},{\"q\":\"🎪 浮世絵は\\nどのようにつくられた？\",\"correct\":\"絵師・彫師・摺師が分かれて作業した\",\"wrongs\":[\"絵師が一枚ずつ手でえがいた\",\"大工と左官がつくった\",\"染物師が布に染めた\"]},{\"q\":\"📚 ききんで米が足りなくなったとき\\n町人が米屋などをおそった行動は？\",\"correct\":\"打ちこわし\",\"wrongs\":[\"百姓一揆\",\"土一揆\",\"徳政令\"]},{\"q\":\"📚 百姓一揆とは\\nどんな行動？\",\"correct\":\"農民が集まって年貢を軽くするよう領主にうったえたこと\",\"wrongs\":[\"町人が米屋をおそったこと\",\"武士が幕府に反乱を起こしたこと\",\"商人が店を閉じたこと\"]},{\"q\":\"📚 徳川吉宗が享保の改革で\\n行ったことは？\",\"correct\":\"新田開発をすすめ質素・倹約を命じた\",\"wrongs\":[\"参勤交代を始めた\",\"鎖国を始めた\",\"刀狩を行った\"]},{\"q\":\"📚 大塩平八郎が大阪で\\n立ち上がったのはなぜ？\",\"correct\":\"ききんで苦しむ人々を救うため\",\"wrongs\":[\"幕府をたおして将軍になるため\",\"キリスト教を広めるため\",\"外国と貿易するため\"]},{\"q\":\"🎪 江戸時代に庶民が旅を\\n楽しめるようになったのはなぜ？\",\"correct\":\"街道や宿場が整い世の中が平和だったから\",\"wrongs\":[\"鉄道が開通したから\",\"馬車が広まったから\",\"関所がなくなったから\"]},{\"q\":\"🎪 相撲や花火など江戸の庶民の\\n楽しみが広がったのはなぜ？\",\"correct\":\"町人が経済力をつけ生活にゆとりができたから\",\"wrongs\":[\"幕府が娯楽を命じたから\",\"武士が仕事をやめたから\",\"外国の文化が入ってきたから\"]},{\"q\":\"📚 国学を学んだ人々が\\n大切にしようとしたのは？\",\"correct\":\"日本に古くから伝わる考え方\",\"wrongs\":[\"中国から伝わった儒学\",\"オランダの医学\",\"キリスト教の教え\"]},{\"q\":\"🎪 松尾芭蕉が「おくのほそ道」で\\n旅した地方は？\",\"correct\":\"東北・北陸地方\",\"wrongs\":[\"九州地方\",\"四国地方\",\"山陰地方\"]}],\"h6u9\":[{\"q\":\"🌅 ペリーの来航の翌年、日本が\\nアメリカと結んだ条約は？\",\"correct\":\"日米和親条約\",\"wrongs\":[\"日米修好通商条約\",\"ポーツマス条約\",\"下関条約\"]},{\"q\":\"🌅 日米修好通商条約が\\n不平等といわれたのはなぜ？\",\"correct\":\"関税を自分で決められず領事裁判権を認めたから\",\"wrongs\":[\"日本が賠償金を払ったから\",\"日本の領土をうばわれたから\",\"日本語を使えなかったから\"]},{\"q\":\"🌅 政権を朝廷に返した\\n江戸幕府の最後の将軍は？\",\"correct\":\"徳川慶喜（とくがわよしのぶ）\",\"wrongs\":[\"徳川家康\",\"徳川吉宗\",\"徳川家光\"]},{\"q\":\"🌅 明治政府が新しい産業を\\n育てようとした政策は？\",\"correct\":\"殖産興業（しょくさんこうぎょう）\",\"wrongs\":[\"廃藩置県\",\"地租改正\",\"四民平等\"]},{\"q\":\"🌅 明治時代に綿糸を\\n大量に生産した工業は？\",\"correct\":\"紡績業（ぼうせきぎょう）\",\"wrongs\":[\"製糸業\",\"製鉄業\",\"造船業\"]},{\"q\":\"🌅 日清戦争のあとに結ばれた\\n講和条約は？\",\"correct\":\"下関条約\",\"wrongs\":[\"ポーツマス条約\",\"日米和親条約\",\"サンフランシスコ平和条約\"]},{\"q\":\"🌅 日清戦争で日本が\\n清から得た領土は？\",\"correct\":\"台湾（たいわん）\",\"wrongs\":[\"朝鮮半島\",\"満州\",\"樺太のすべて\"]},{\"q\":\"🌅 日露戦争のとき「君死にたまふこと\\nなかれ」とうたった歌人は？\",\"correct\":\"与謝野晶子（よさのあきこ）\",\"wrongs\":[\"樋口一葉\",\"津田梅子\",\"平塚らいてう\"]},{\"q\":\"🌅 破傷風の治療法を見つけ\\n伝染病の研究所をつくった医学者は？\",\"correct\":\"北里柴三郎（きたさとしばさぶろう）\",\"wrongs\":[\"野口英世\",\"杉田玄白\",\"森鴎外\"]},{\"q\":\"🌅 アフリカで黄熱病の研究に\\n取り組んだ医学者は？\",\"correct\":\"野口英世（のぐちひでよ）\",\"wrongs\":[\"北里柴三郎\",\"志賀潔\",\"津田梅子\"]},{\"q\":\"🌅 足尾銅山の公害を\\n国にうったえ続けた人物は？\",\"correct\":\"田中正造（たなかしょうぞう）\",\"wrongs\":[\"板垣退助\",\"福沢諭吉\",\"大久保利通\"]},{\"q\":\"🌅 官営の八幡製鉄所が\\nつくられた県は？\",\"correct\":\"福岡県\",\"wrongs\":[\"群馬県\",\"大阪府\",\"北海道\"]},{\"q\":\"🌅 自由民権運動が\\n求めたことは？\",\"correct\":\"国会を開き国民が政治に参加すること\",\"wrongs\":[\"幕府を復活させること\",\"外国と戦争すること\",\"士農工商をもどすこと\"]}],\"h6u10\":[{\"q\":\"💥 第一次世界大戦のころ\\n日本の経済はどうなった？\",\"correct\":\"輸出が増えて好景気になった\",\"wrongs\":[\"不景気で工場が閉じた\",\"ほとんど変わらなかった\",\"米がとれず飢きんになった\"]},{\"q\":\"💥 1918年に米の値段が上がり\\n各地で起きたさわぎは？\",\"correct\":\"米騒動（こめそうどう）\",\"wrongs\":[\"打ちこわし\",\"百姓一揆\",\"学生運動\"]},{\"q\":\"💥 大正時代に「新婦人協会」をつくり\\n女性の権利を求めたのは？\",\"correct\":\"平塚らいてう\",\"wrongs\":[\"与謝野晶子\",\"津田梅子\",\"樋口一葉\"]},{\"q\":\"💥 1923年の関東大震災のあと\\n世の中はどうなった？\",\"correct\":\"多くの命と家が失われ不景気が深まった\",\"wrongs\":[\"復興ですぐに好景気になった\",\"戦争が終わった\",\"選挙権が広がった\"]},{\"q\":\"💥 満州事変のあと日本が\\n満州につくった国は？\",\"correct\":\"満州国（まんしゅうこく）\",\"wrongs\":[\"中華民国\",\"朝鮮国\",\"台湾国\"]},{\"q\":\"💥 日本が国際連盟を\\n脱退したのはなぜ？\",\"correct\":\"満州国を認めてもらえなかったから\",\"wrongs\":[\"戦争に負けたから\",\"お金が足りなかったから\",\"国際連盟がなくなったから\"]},{\"q\":\"💥 戦争が長引くと\\n学校の生活はどうなった？\",\"correct\":\"勉強より工場などで働くことが中心になった\",\"wrongs\":[\"英語の授業が増えた\",\"授業時間が長くなった\",\"休みが増えた\"]},{\"q\":\"💥 戦争中に生活の品物が\\n足りなくなったのはなぜ？\",\"correct\":\"物資も人も戦争に優先して使われたから\",\"wrongs\":[\"外国が買い占めたから\",\"農家が作るのをやめたから\",\"値段が高すぎたから\"]},{\"q\":\"💥 空襲からのがれるため\\n家の近くにほられたものは？\",\"correct\":\"防空壕（ぼうくうごう）\",\"wrongs\":[\"井戸\",\"用水路\",\"貯水池\"]},{\"q\":\"💥 原子爆弾が落とされたあと\\n長く人々を苦しめたものは？\",\"correct\":\"放射線による病気（原爆症）\",\"wrongs\":[\"台風\",\"伝染病のコレラ\",\"食中毒\"]},{\"q\":\"💥 ポツダム宣言を受け入れるとは\\nどういうこと？\",\"correct\":\"日本が戦争に負けたことを認めること\",\"wrongs\":[\"戦争を続けること\",\"中国と同盟を結ぶこと\",\"新しい憲法をつくること\"]},{\"q\":\"💥 日本が中国や東南アジアの人々に\\n与えた被害について正しいのは？\",\"correct\":\"多くの人が命をうばわれくらしをこわされた\",\"wrongs\":[\"被害はほとんどなかった\",\"経済が豊かになった\",\"日本が賠償金を受け取った\"]},{\"q\":\"💥 1945年8月、ソ連が\\n日本に対して行ったことは？\",\"correct\":\"満州などに攻めこんだ\",\"wrongs\":[\"日本を助けて参戦した\",\"平和条約を結んだ\",\"原子爆弾を落とした\"]},{\"q\":\"💥 戦争中の新聞やラジオが\\n伝えた情報の特ちょうは？\",\"correct\":\"国に都合のよいことだけが伝えられた\",\"wrongs\":[\"何でも自由に伝えられた\",\"外国の新聞がそのまま読めた\",\"情報がまったくなかった\"]},{\"q\":\"💥 疎開した子どもたちの\\n生活はどうだった？\",\"correct\":\"家族とはなれ食べ物も足りずさびしい思いをした\",\"wrongs\":[\"家族と一緒で安心してくらせた\",\"都会より豊かにくらせた\",\"学校に通わず遊んでばかりだった\"]},{\"q\":\"💥 東京大空襲で被害が\\n大きくなったのはなぜ？\",\"correct\":\"木造の家が多く焼夷弾で火が広がったから\",\"wrongs\":[\"地震が同時に起きたから\",\"川がなかったから\",\"人が少なかったから\"]},{\"q\":\"💥 戦争が終わったとき\\n日本の多くの都市はどうなっていた？\",\"correct\":\"空襲で焼け野原になっていた\",\"wrongs\":[\"ほとんど被害がなかった\",\"外国軍に占領されず無事だった\",\"すでに復興していた\"]}],\"h6u11\":[]};var _H6CACHE={};function _H6NK(s){s=String(s);var o='';for(var i=0;i<s.length;i++){var c=s.charCodeAt(i);if((c>=0x3041&&c<=0x30FF)||(c>=0x4E00&&c<=0x9FFF)||(c>=0x30&&c<=0x39)||(c>=0x41&&c<=0x5A)||(c>=0x61&&c<=0x7A)||(c>=0xFF10&&c<=0xFF19))o+=s.charAt(i);}return o;}function _H6M(k){if(_H6CACHE[k])return _H6CACHE[k];var out=[],seen={},ds={},dl=_H6DROP[k]||[],i;for(i=0;i<dl.length;i++)ds[dl[i]]=1;var put=function(it,chk){if(!it||!it.q)return;var n=_H6NK(it.q);if(seen[n])return;if(chk&&ds[n])return;seen[n]=1;out.push(it);};var ss=(typeof _SS!=='undefined'&&_SS&&_SS[k])?_SS[k]:[];for(i=0;i<ss.length;i++)put(ss[i],false);var sc=(typeof _SC!=='undefined'&&_SC&&_SC[k])?_SC[k]:[];for(i=0;i<sc.length;i++)put(sc[i],true);var ex=_H6ADD[k]||[];for(i=0;i<ex.length;i++)put(ex[i],false);if(!out.length)out=ss.slice();_H6CACHE[k]=out;return out;}function genH6u1(){return _pickBank(_H6M('h6u1'));}function genH6u2(){return _pickBank(_H6M('h6u2'));}function genH6u3(){return _pickBank(_H6M('h6u3'));}function genH6u4(){return _pickBank(_H6M('h6u4'));}function genH6u5(){return _pickBank(_H6M('h6u5'));}function genH6u6(){return _pickBank(_H6M('h6u6'));}function genH6u7(){return _pickBank(_H6M('h6u7'));}function genH6u8(){return _pickBank(_H6M('h6u8'));}function genH6u9(){return _pickBank(_H6M('h6u9'));}function genH6u10(){return _pickBank(_H6M('h6u10'));}function genH6u11(){return _pickBank(_H6M('h6u11'));}")
      // __H6U11_BOOST_V1__ : u11（新しい日本へ）追加34問 + 誤答差し替え10問
      t = t.replace("function genH6u11(){return _pickBank(_H6M('h6u11'));}", "function genH6u11(){return _pickBank(_H6M('h6u11'));};(function(){try{if(typeof _H6ADD==='undefined'||typeof _H6NK!=='function'){return;}var K='h6u11';var _H6U11FIX_V1=[[\"日本国憲法の三大原則は\", null, [\"国民主権・基本的人権の\\n尊重・三権分立\", \"教育・勤労・納税\", \"立法・行政・司法\"]], [\"1946年に公布された新しい憲法は\", null, [\"大日本帝国憲法\", \"教育基本法\", \"ポツダム宣言\"]], [\"日本国憲法が施行された年は\", null, [\"1946年11月3日\", \"1945年8月15日\", \"1947年11月3日\"]], [\"GHQが行った民主化の改革は\", null, [\"地租改正・徴兵令・学制\", \"廃藩置県・四民平等\", \"殖産興業・富国強兵\"]], [\"農地改革とは\", null, [\"大きな会社（財閥）を\\n分けて競争させたこと\", \"土地に税をかけて\\n現金で納めさせたこと\", \"田畑の広さを調べて\\n年貢の量を決めたこと\"]], [\"公害問題が深刻になった高度成長期に有名な病気は\", null, [\"足尾銅山の鉱毒による\\n被害\", \"結核や脚気\", \"コレラや赤痢\"]], [\"現在の日本の政治の形は\", null, [\"天皇が主権を持つ\\n仕組み\", \"軍隊が政治を動かす\\n仕組み\", \"一つの政党だけが\\n政治を行う仕組み\"]], [\"基本的人権とは\", null, [\"選挙で代表を選ぶ権利\", \"国民が果たさなければ\\nならない義務\", \"国会が決めて\\n国民にあたえる権利\"]], [\"戦後の教育改革で変わったことは\", null, [\"義務教育が小学校の\\n6年間だけになった\", \"男子と女子が別々の\\n学校で学ぶことになった\", \"6年・5年・3年の\\n学校制度になった\"]], [\"日本国憲法が大切にしている三つの権利に含まれるのは\", null, [\"国民主権・基本的人権の\\n尊重・平和主義\", \"教育・勤労・納税\", \"言論の自由・出版の自由・\\n集会の自由\"]]];var _H6U11ADD_V1=[{\"q\": \"🕊️ 憲法記念日は\\n何月何日？\", \"correct\": \"5月3日\", \"wrongs\": [\"11月3日\", \"8月15日\", \"4月29日\"]}, {\"q\": \"🕊️ 11月3日の「文化の日」は\\n憲法の何にちなむ日？\", \"correct\": \"日本国憲法が\\n公布された日\", \"wrongs\": [\"日本国憲法が\\n施行された日\", \"戦争が終わった日\", \"国会が初めて\\n開かれた日\"]}, {\"q\": \"🕊️ 日本国憲法で\\n天皇はどのような\\n立場とされている？\", \"correct\": \"日本の国と国民の\\nまとまりの象徴\", \"wrongs\": [\"国の政治を行う\\n最高の責任者\", \"軍隊を指揮する\\n総大将\", \"国会で法律を決める人\"]}, {\"q\": \"🕊️ 日本国憲法が定める\\n国民の三つの義務は？\", \"correct\": \"教育を受けさせる義務・\\n勤労の義務・納税の義務\", \"wrongs\": [\"選挙に行く義務・\\n裁判を受ける義務・\\n納税の義務\", \"教育・勤労・兵役の\\n三つの義務\", \"国を守る義務・働く義務・\\n税を納める義務\"]}, {\"q\": \"🕊️ 日本国憲法を\\n改正するときに\\n必要なことは？\", \"correct\": \"国会が決めたあと\\n国民投票で決める\", \"wrongs\": [\"内閣だけで決める\", \"天皇が決める\", \"最高裁判所が決める\"]}, {\"q\": \"🕊️ 戦争を放棄すると\\n定めているのは\\n憲法の第何条？\", \"correct\": \"第9条\", \"wrongs\": [\"第1条\", \"第11条\", \"第25条\"]}, {\"q\": \"🕊️ 日本の\\n「非核三原則」の\\n内容は？\", \"correct\": \"核兵器を持たず・\\n作らず・持ちこませず\", \"wrongs\": [\"核兵器を持たず・\\n作らず・使わず\", \"核兵器を作らず・\\n売らず・買わず\", \"核兵器を持たず・\\n売らず・捨てず\"]}, {\"q\": \"🕊️ 大日本帝国憲法で\\n主権を持っていたのは\\n誰？\", \"correct\": \"天皇\", \"wrongs\": [\"国民\", \"国会\", \"内閣総理大臣\"]}, {\"q\": \"🕊️ 戦後の選挙で\\n新しく認められた\\nことは？\", \"correct\": \"女性も投票できる\\nようになった\", \"wrongs\": [\"25歳以上の男性だけが\\n投票できるようになった\", \"税を多く納めた人だけが\\n投票できるようになった\", \"投票できる年齢が\\n30歳に上げられた\"]}, {\"q\": \"🕊️ 1945年に選挙権が\\nあたえられたのは\\n何歳以上の男女？\", \"correct\": \"20歳以上\", \"wrongs\": [\"25歳以上\", \"18歳以上\", \"30歳以上\"]}, {\"q\": \"🕊️ 1945年8月15日は\\n何の日？\", \"correct\": \"戦争が終わったことが\\n国民に知らされた日\", \"wrongs\": [\"広島に原子爆弾が\\n落とされた日\", \"ポツダム宣言が\\n出された日\", \"日本国憲法が\\n公布された日\"]}, {\"q\": \"🕊️ 広島に原子爆弾が\\n落とされた日は？\", \"correct\": \"1945年8月6日\", \"wrongs\": [\"1945年8月9日\", \"1945年8月15日\", \"1945年7月26日\"]}, {\"q\": \"🕊️ 長崎に原子爆弾が\\n落とされた日は？\", \"correct\": \"1945年8月9日\", \"wrongs\": [\"1945年8月6日\", \"1945年8月15日\", \"1945年9月2日\"]}, {\"q\": \"🕊️ 日本が受け入れて\\n降伏を決めた宣言は？\", \"correct\": \"ポツダム宣言\", \"wrongs\": [\"サンフランシスコ\\n平和条約\", \"日米安全保障条約\", \"五箇条の御誓文\"]}, {\"q\": \"🕊️ 戦後すぐ、学校の\\n教科書はどうなった？\", \"correct\": \"軍国主義の部分を\\n墨で黒くぬって使った\", \"wrongs\": [\"すぐに新しい教科書に\\nすべて取りかえた\", \"そのまま何も変えずに\\n使い続けた\", \"教科書を使うことが\\n禁止された\"]}, {\"q\": \"🕊️ 戦後、食料が不足し\\n都市の人が農村へ\\n買いに行ったことを\\n何という？\", \"correct\": \"買い出し\", \"wrongs\": [\"配給\", \"疎開\", \"開拓\"]}, {\"q\": \"🕊️ 「財閥解体」とは？\", \"correct\": \"経済を支配していた\\n大きな会社を分けて\\n競争できるようにしたこと\", \"wrongs\": [\"地主の土地を農民に\\n分けあたえたこと\", \"会社をすべて国のものに\\nしたこと\", \"外国の会社を\\n追い出したこと\"]}, {\"q\": \"🕊️ 戦後、男女が同じ\\n教室で学ぶように\\nなったことを何という？\", \"correct\": \"男女共学\", \"wrongs\": [\"男女別学\", \"六・三制\", \"義務教育\"]}, {\"q\": \"🕊️ 戦後、義務教育は\\n何年間になった？\", \"correct\": \"9年間\", \"wrongs\": [\"6年間\", \"12年間\", \"4年間\"]}, {\"q\": \"🕊️ 農地改革によって\\n増えたのはどんな\\n農民？\", \"correct\": \"自分の土地を持って\\n耕す農民\", \"wrongs\": [\"土地を借りて耕す農民\", \"広い土地を持つ地主\", \"土地を持たない武士\"]}, {\"q\": \"🕊️ 高度経済成長のころ\\n「三種の神器」と\\n呼ばれたものは？\", \"correct\": \"白黒テレビ・洗濯機・\\n冷蔵庫\", \"wrongs\": [\"カラーテレビ・クーラー・\\n自動車\", \"パソコン・携帯電話・\\nエアコン\", \"ラジオ・電話・自転車\"]}, {\"q\": \"🕊️ 1970年に大阪で\\n開かれた国際的な\\n催しは？\", \"correct\": \"日本万国博覧会\", \"wrongs\": [\"東京オリンピック\", \"札幌オリンピック\", \"国際連合の総会\"]}, {\"q\": \"🕊️ 1964年の東京\\nオリンピックに\\n合わせて開通した\\n鉄道は？\", \"correct\": \"東海道新幹線\", \"wrongs\": [\"東北新幹線\", \"山陽新幹線\", \"青函トンネルの鉄道\"]}, {\"q\": \"🕊️ 高度経済成長のころ\\n都市に人が集まり\\n農村で人が減った\\nことを何という？\", \"correct\": \"過密と過疎\", \"wrongs\": [\"集団就職\", \"文明開化\", \"産業革命\"]}, {\"q\": \"🕊️ 公害を防ぐために\\n1971年につくられた\\n国の役所は？\", \"correct\": \"環境庁\", \"wrongs\": [\"厚生省\", \"文部省\", \"通商産業省\"]}, {\"q\": \"🕊️ 四大公害病のうち\\n熊本県で発生した\\n病気は？\", \"correct\": \"水俣病\", \"wrongs\": [\"イタイイタイ病\", \"四日市ぜんそく\", \"新潟水俣病\"]}, {\"q\": \"🕊️ イタイイタイ病が\\n発生した県は？\", \"correct\": \"富山県\", \"wrongs\": [\"熊本県\", \"三重県\", \"新潟県\"]}, {\"q\": \"🕊️ 四日市ぜんそくの\\n主な原因は？\", \"correct\": \"工場から出るけむりに\\nよる空気のよごれ\", \"wrongs\": [\"工場から流された水銀\", \"鉱山から流された\\nカドミウム\", \"川にすてられたごみ\"]}, {\"q\": \"🕊️ 水俣病の主な原因は？\", \"correct\": \"工場から流された水銀で\\nよごれた魚や貝を\\n食べたこと\", \"wrongs\": [\"工場から流された\\nカドミウムでよごれた\\n米を食べたこと\", \"工場から出るけむりを\\n吸い続けたこと\", \"農薬を使いすぎたこと\"]}, {\"q\": \"🕊️ 1972年に日本が\\n国交を正常化した\\n国は？\", \"correct\": \"中国\", \"wrongs\": [\"ソ連\", \"韓国\", \"アメリカ\"]}, {\"q\": \"🕊️ 現在もアメリカ軍の\\n基地が多く残って\\nいる県は？\", \"correct\": \"沖縄県\", \"wrongs\": [\"北海道\", \"長崎県\", \"東京都\"]}, {\"q\": \"🕊️ 1973年に起こり\\n高度経済成長が\\n終わるきっかけと\\nなったできごとは？\", \"correct\": \"石油危機\", \"wrongs\": [\"バブル経済の崩壊\", \"世界恐慌\", \"関東大震災\"]}, {\"q\": \"🕊️ 日本の平和と安全を\\n守るためにつくられた\\n組織は？\", \"correct\": \"自衛隊\", \"wrongs\": [\"陸軍・海軍\", \"国際連合軍\", \"GHQ\"]}, {\"q\": \"🕊️ 内閣総理大臣は\\nどのように決まる？\", \"correct\": \"国会で指名される\", \"wrongs\": [\"国民が直接選挙で選ぶ\", \"天皇が指名する\", \"裁判所が選ぶ\"]}];var mp={},i,j;for(i=0;i<_H6U11FIX_V1.length;i++){mp[_H6NK(_H6U11FIX_V1[i][0])]=_H6U11FIX_V1[i];}var pools=[];if(typeof _SS!=='undefined'&&_SS&&_SS[K]){pools.push(_SS[K]);}if(typeof _SC!=='undefined'&&_SC&&_SC[K]){pools.push(_SC[K]);}for(i=0;i<pools.length;i++){var arr=pools[i];for(j=0;j<arr.length;j++){var f=mp[_H6NK(arr[j].q)];if(f){if(f[1]){arr[j].correct=f[1];}arr[j].wrongs=f[2].slice();}}}_H6ADD[K]=(_H6ADD[K]||[]).concat(_H6U11ADD_V1);if(typeof _H6CACHE!=='undefined'&&_H6CACHE){delete _H6CACHE[K];}}catch(e){}})();")

      // __DEFLANE_MIX_V1__ 防衛戦だけ てきのレーンを seed から きめる（ジムチャレンジは foeLaneMix を わたさないので これまでどおり）
      t = t.replace("for(var _cj=0;_cj<B.length;_cj++){ B[_cj].adv=0; B[_cj].curLn=(B[_cj].lane!=null?B[_cj].lane:(_cj%CLN)); } }", "/* __DEFLANE_MIX_V1__ 防衛戦だけ: てきのレーンを seed から きめる。強い順に 1体ずつ くばるので 1つの道に かたよらない。 */ var _cLB=null; if(opts.foeLaneMix){ var _cO=[],_cx,_cy,_ct; for(_cx=0;_cx<B.length;_cx++) _cO.push(_cx); _cO.sort(function(x,y){ var sx=(Number(B[x].maxHp)||0)+3*(Number(B[x].atk)||0), sy=(Number(B[y].maxHp)||0)+3*(Number(B[y].atk)||0); return (sy-sx)||(x-y); }); _cLB=new Array(B.length); var _cL2=[]; for(_cx=0;_cx<CLN;_cx++) _cL2.push(_cx); for(var _cr=0;_cr*CLN<B.length;_cr++){ for(_cx=CLN-1;_cx>0;_cx--){ _cy=Math.floor(rng()*(_cx+1)); _ct=_cL2[_cx]; _cL2[_cx]=_cL2[_cy]; _cL2[_cy]=_ct; } for(_cx=0;_cx<CLN&&_cr*CLN+_cx<B.length;_cx++) _cLB[_cO[_cr*CLN+_cx]]=_cL2[_cx]; } } for(var _cj=0;_cj<B.length;_cj++){ B[_cj].adv=0; B[_cj].curLn=(B[_cj].lane!=null?B[_cj].lane:(_cLB?_cLB[_cj]:(_cj%CLN))); } }")
      // __DEF_YOUTOO_V1__ まだ 出していない子に やさしく つたえる（せめない言い方で）
      t = t.replace(`if(d.my_entry && d.my_entry.monster){ var me=d.my_entry.monster; head+='<div style="background:#ecfdf5;`, `if(!(d.my_entry && d.my_entry.monster)){ head+='<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:10px;margin-bottom:8px;"><div style="font-weight:900;color:#1d4ed8;">🤝 きみも たたかっているよ</div><div style="font-size:13px;color:#334155;margin-top:3px;line-height:1.6;">クラスの みんなが 出るので、きみの いちばん上の モンスターも いっしょに たたかうよ。<br>プログラムを くむと、クラスは もっと つよくなる。</div></div>'; } if(d.my_entry && d.my_entry.monster){ var me=d.my_entry.monster; head+='<div style="background:#ecfdf5;`)
    // __ZWAR_UNLOCK_V1__ : ゾンビ襲来の解禁判定をステージ選択からボタン側へ移す
    const _zwA1 = "let zUnlocked = 1;\n    if(isZombie){\n      let c = 0;\n      while(c < WAR_PREFS.length && zCleared[String(c)]) c++;\n      zUnlocked = clamp(c + 1, 1, WAR_PREFS.length);\n    }"
    const _zwA2 = "let zUnlocked = (progress.unlocked || 1);"
    if (t.indexOf(_zwA1) !== -1) { t = t.replace(_zwA1, _zwA2) } else { console.error('[__ZWAR_UNLOCK_V1__] anchor A not found') }
    const _zwB1 = "let maxUnlocked = (player.warProgress.unlocked||1);\n    try{\n      const isZombie = !!(window.warState && window.warState.zombieMode);\n      if(isZombie){\n        const z = (player.warProgress && player.warProgress.zombieCleared && typeof player.warProgress.zombieCleared==='object') ? player.warProgress.zombieCleared : {};\n        let c = 0;\n        while(c < WAR_PREFS.length && z[String(c)]) c++;\n        maxUnlocked = clamp(c + 1, 1, WAR_PREFS.length);\n      }\n    }catch(e){}"
    const _zwB2 = "let maxUnlocked = (player.warProgress.unlocked||1);"
    if (t.indexOf(_zwB1) !== -1) { t = t.replace(_zwB1, _zwB2) } else { console.error('[__ZWAR_UNLOCK_V1__] anchor B not found') }
...
/* 🗑 2026-09: openStudentKarte()（Geminiで個人カルテを書く方）は削除しました。
      ...
      async function copyUnifiedAi(){
        ...
        if(optKarte) L.push('・=== [KARTE:児童ID] 名前 === … 阪神マン風（関西弁・前向き）で ①ええところ ②気になるところ ③おすすめの学習・声かけ。今年度（4月〜）全期間の傾向と成果物の評価（◎○△）も踏まえ、課題は正直に＋次の一歩とセットで。※児童向けカルテ本文には◎○△などの記号は書かず、言葉で励ます。');
      ...
      function copyTestPrompt(){
        ...
        fetch('/api/teacher/class/'+encodeURIComponent(cid)+'/members').then(function(r){return r.json();}).then(function(d){
          ...
          var stxt=subject||'（教科はテスト内容からAIが判定）';
          var utxt=unit?('・単元「'+unit+'」'):'';
          var L=[];
          L.push('あなたは小学校の先生を助けるアシスタントです。これは【'+gtxt+'・'+stxt+utxt+'】のテスト／プリントです。アップロードした画像・PDFから、児童ごとに (1)点数 (2)'+gtxt+'の評価基準（観点別評価）に照らした評価 を読み取り／判定し、次の「出力形式」だけをそのまま出力してください。前置き・説明・コードブロックは書かないでください。');
          L.push('');
          ...
          L.push('');
          var _evL='【'+gtxt+' '+stxt+(unit?(' '+unit):'')+'】';
          var _skMap={'算数':'算数の例：計算や数量・図形の理解と手順（知識・技能）、問題の解き方・理由の説明・式や図での表現（思考・判断・表現）。','国語':'国語の例：漢字・語句・音読や読みの正確さ（知識・技能）、要点や登場人物の気持ちの読み取り・自分の考えの記述（思考・判断・表現）。','理科':'理科の例：用語・器具の使い方・観察や実験の技能（知識・技能）、予想・考察・結果からの説明（思考・判断・表現）。','社会':'社会の例：地図・資料・用語の理解（知識・技能）、資料の読み取り・理由や関係の説明（思考・判断・表現）。','英語':'英語の例：語句や表現の理解（知識・技能）、聞く・話す・書くでのやり取りや表現（思考・判断・表現）。'};
          var _sk=_skMap[subject]||'';
          var _EK={
          ...
          L.push('【評価のつけ方】'+_evL+'の評価基準（学習指導要領の「観点別評価」の考え方）に照らして、3段階でつけます。学年が上がるほど求める到達度も上がることを反映してください。');
          L.push('観点（この学年・教科・単元でわかる範囲で）：①知識・技能（用語・計算・手順が正しく身についているか）②思考・判断・表現（考え方・理由の説明・図や式での表現ができているか）③主体的に学習に取り組む態度（答案の取り組み・見直し・工夫からわかる範囲で）。');
          if(subject){
          ...
          L.push('◎ … その学年の評価基準を十分に満たす（観点をよく満たしている）');
          L.push('○ … その学年の評価基準をおおむね満たす');
          L.push('△ … 一部のみ到達・支援が必要（基礎的な観点につまずきがある）');
          L.push('※点数だけで機械的に決めず、上の観点と'+gtxt+'の到達基準に照らして総合的に判断してください。答案の中身が見えるときは観点の様子（例：計算はできるが理由の説明が弱い）も踏まえます。点数が読み取れないときは、答案の出来ぐあいから評価だけをつけてください（点数は空欄でよい）。');
          L.push('');
          L.push('【出力形式】（評価は観点別に3つ。知技=知識・技能／思判表=思考・判断・表現／主体=主体的に学習に取り組む態度。各観点は ◎○△ で、そのテストで測っていない観点は空欄にします）');
          L.push('テスト名: （テストの名前）');
          ...
          L.push('児童名, 点数, 知技, 思判表, 主体, コメント');
          L.push('');
          L.push('【ルール】各項目はカンマ（,）で区切り、1行に1人。点数は数字のみ（不明は空欄）。知技・思判表・主体はそれぞれ ◎ ○ △ か空欄（そのテストで測れない観点は空欄）。コメントは20文字以内で、'+gtxt+'の評価規準に照らして「できている観点／次に伸ばす観点」を一言で。児童名は必ず上の名簿の表記に合わせる。合計・平均などの余計な行は入れない。');
          var txt=L.join(NL);
