/* タイプシュート 試作版 v0.1（1ステージ・タイピングで撃つ）
 * 独立モード。既存のバトルには干渉しない。ローマ字でことばを打つ→自分のモンスターが発射。
 */
(function () {
  'use strict';

  // ===== ローマ字テーブル（1文字かな → 打ち方の候補）=====
  var ROMA = {
    'あ':['a'],'い':['i'],'う':['u'],'え':['e'],'お':['o'],
  ...
  // 単語(かな)→ 許容できるローマ字の全パターン（短い語のみ・組合せは少数）
  function romaPatterns(word) {
    var sets = [];
    for (var i = 0; i < word.length; i++) {
      var k = ROMA[word[i]];
      if (!k) return [word]; // 未対応文字はそのまま
      sets.push(k);
    }
    var out = [''];
    for (var s = 0; s < sets.length; s++) {
      ...
      for (var a = 0; a < out.length; a++) {
        for (var b = 0; b < sets[s].length; b++) next.push(out[a] + sets[s][b]);
      }
      out = next;
  ...
  function isPrefix(typed, pats) { for (var i = 0; i < pats.length; i++) if (pats[i].indexOf(typed) === 0) return true; return false; }
  function isComplete(typed, pats) { for (var i = 0; i < pats.length; i++) if (pats[i] === typed) return true; return false; }

  // ===== World1 ことば（試作：小さい字・っ・伸ばしは未使用）=====
  var WORDS = ['ねこ','いぬ','とり','うさぎ','きりん','ぱんだ','さかな','からす',
    'りんご','みかん','ばなな','すいか','とまと','ぶどう',
    'えんぴつ','つくえ','はさぽ','ともだち','せんせい','たいいく','ひまわり'];

  // ===== 状態 =====
  var S = { open:false, word:'', pats:[], typed:'', myHp:100, cpuHp:100, combo:0,
    raf:null, missiles:[], cpuTimer:null, ended:false, last:0 };

  function getMyMonster() {
    try {
      var id = window.player && window.player.party && window.player.party[0];
      var list = window.MONSTERS || [];
      for (var i = 0; i < list.length; i++) if (list[i].id === id) return { sprite: list[i].sprite || '⭐' };
    } catch (e) {}
    return { sprite: '⭐' };
  }
  function giveCoins(n) {
    try {
      if (window.player) {
        window.player.coins = (window.player.coins || 0) + n;
        if (typeof window.player.oldCoinDailyStart !== 'undefined') window.player.oldCoinDailyStart = window.player.coins;
      }
  ...
  function build() {
    ...
    o.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#0f172a">' +
        '<div style="font-weight:700;color:#a78bfa">⌨ タイプシュート（しさく版）</div>' +
      ...
      '<div style="padding:8px 14px">' +
        '<div style="font-size:12px;color:#94a3b8">あいて の きち</div>' +
        '<div style="background:#1e293b;border-radius:8px;height:14px;overflow:hidden;margin-top:3px"><div id="tsCpuBar" style="height:14px;background:#ef4444;width:100%;transition:width .3s"></div></div>' +
      '</div>' +
      ...
      '<div style="padding:6px 14px">' +
        '<div style="font-size:12px;color:#94a3b8">じぶん の きち</div>' +
        '<div style="background:#1e293b;border-radius:8px;height:14px;overflow:hidden;margin-top:3px"><div id="tsMyBar" style="height:14px;background:#4ade80;width:100%;transition:width .3s"></div></div>' +
      '</div>' +
      '<div style="padding:10px 14px 20px;text-align:center;background:#0f172a">' +
  ...
  function spawnMissile(dir, emoji) {
    ...
    m.textContent = emoji;
    m.style.cssText = 'position:absolute;font-size:24px;left:' + x + 'px;top:' + startY + 'px;transition:none';
    f.appendChild(m);
    S.missiles.push({ node: m, dir: dir, y: startY, x: x, h: f.clientHeight });
  }
  function loop(ts) {
    ...
    for (var i = S.missiles.length - 1; i >= 0; i--) {
      var mo = S.missiles[i];
      var speed = 0.22 * dt; // px/ms
      mo.y += mo.dir === 'up' ? -speed : speed;
      mo.node.style.top = mo.y + 'px';
      if (mo.dir === 'up' && mo.y <= 26) { hitCpu(); remove(i, mo); }
      else if (mo.dir === 'down' && mo.y >= fh - 40) { hitMe(); remove(i, mo); }
    }
    S.raf = requestAnimationFrame(loop);
  }
  function remove(i, mo) { try { mo.node.remove(); } catch (e) {} S.missiles.splice(i, 1); }

  function hitCpu() { S.cpuHp = Math.max(0, S.cpuHp - 20); setBars(); if (S.cpuHp <= 0) finish(true); }
  ...
  function cpuFire() { if (!S.open || S.ended) return; spawnMissile('down', '💧'); }
    ...
    if (e.key && e.key.length === 1 && /[a-zA-Z\-]/.test(e.key)) {
      ...
      if (isPrefix(t, S.pats)) {
        ...
        if (isComplete(S.typed, S.pats)) {
          ...
          spawnMissile('up', getMyMonster().sprite);
          setWord();
        }
      } else {
  ...
  function startGame() {
    build();
    S.open = true; S.ended = false; S.myHp = 100; S.cpuHp = 100; S.combo = 0; S.missiles = []; S.last = 0;
  ...
  function finish(win) {
    if (S.ended) return; S.ended = true; stopLoops();
    var reward = win ? 30 : 0;
    if (win) giveCoins(reward);
    var r = el('tsResult');
    ...
    el('tsRetry').onclick = startGame;
    el('tsBack').onclick = closeGame;
  }
  function closeGame() {
    S.open = false; stopLoops();
  ...
  console.log('[TypeShoot v0.1] loaded');
})();
