...
- NEVER include the student's name in the comment. The name is already displayed separately.
...
- Use encouraging, warm tone like a real teacher writing in a notebook (e.g. "算数がんばったね！" "41分も集中できてすごい！")
...
=== Student Reflections ===
${n.results.map((u,m)=>{const b=i[u.user_id]||[],w=b.length?b.map(y=>`  [${y.day_key}] ${y.todo||""}(${y.minutes||0}分) 先生:${y.teacher_comment||"なし"}`).join(`
...
`).filter(g=>g.match(/^\d+[\.\)]/)).map(g=>g.replace(/^\d+[\.\)]\s*/,"").trim())}const w=n.results.map((g,y)=>({id:g.id,name:g.name,dayKey:g.day_key,comment:(b[y]||"").slice(0,50)}));return e.json({ok:!0,comments:w})}catch(u){return o(e,500,"AI error: "+(u.message||String(u)))}});function J(e){const t=new Date,s=new Date(Date.UTC(t.getFullYear(),t.getMonth(),t.getDate()));s.setUTCDate(s.getUTCDate()+4-(s.getUTCDay()||7));const a=new Date(Date.UTC(s.getUTCFullYear(),0,1)),n=Math.ceil(((s.getTime()-a.getTime())/864e5+1)/7);return`${s.getUTCFullYear()}-W${String(n).padStart(2,"0")}`}function Ge(e){const t=e.match(/^(\d{4})-W(\d{2})$/);if(!t)return e;let s=parseInt(t[1]),a=parseInt(t[2]);return a--,a<1&&(s--,a=52),s+"-W"+String(a).padStart(2,"0")}h.post("/api/teacher/class/:classId/weekly-menu",async e=>{const t=e.get("user");if(!t||t.role!=="teacher"&&t.role!=="admin")return o(e,403,"forbidden");const s=e.req.param("classId");if(!(t.role==="admin"?await e.env.DB.prepare("SELECT id FROM classes WHERE id=? LIMIT 1").bind(s).first():await e.env.DB.prepare("SELECT id FROM classes WHERE id=? AND teacher_id=? LIMIT 1").bind(s,t.id).first()))return o(e,404,"class_not_found");const r=await e.req.json().catch(()=>null);if(!r)return o(e,400,"invalid_json");const i=String(r.weekKey||J()).slice(0,8),d=String(r.kanjiPage||"").slice(0,100),l=String(r.keisanPage||"").slice(0,100),c=String(r.otherTasks||"").slice(0,500),u=String(r.tests||"").slice(0,500),m=["mon","tue","wed","thu","fri"],b=Array.isArray(r.activeDays)?r.activeDays.filter(g=>m.includes(g)):m,w=JSON.stringify(b);return await e.env.DB.prepare(`
  ...
  `).bind(s,i,d,l,c,u,w,Date.now()).run(),e.json({ok:!0,weekKey:i})});h.get("/api/teacher/class/:classId/weekly-menu",async e=>{const t=e.get("user");if(!t||t.role!=="teacher"&&t.role!=="admin")return o(e,403,"forbidden");const s=e.req.param("classId");if(!(t.role==="admin"?await e.env.DB.prepare("SELECT id FROM classes WHERE id=? LIMIT 1").bind(s).first():await e.env.DB.prepare("SELECT id FROM classes WHERE id=? AND teacher_id=? LIMIT 1").bind(s,t.id).first()))return o(e,404,"class_not_found");const r=e.req.query("weekKey")||J(),i=await e.env.DB.prepare("SELECT * FROM class_weekly_menu WHERE class_id=? AND week_key=? LIMIT 1").bind(s,r).first();return e.json({ok:!0,menu:i||null,weekKey:r})});h.get("/api/student/weekly-menu",async e=>{const t=e.get("user");if(!t)return o(e,403,"forbidden");const s=e.req.query("weekKey")||J(),a=e.req.query("classId")||"";let n=null;if(t.role==="teacher"||t.role==="admin"?a?n=await e.env.DB.prepare(`
      ...
      LIMIT 1
    `).bind(t.id,s).first(),!n){const r=Ge(s);let i=null;return t.role==="teacher"||t.role==="admin"?a?i=await e.env.DB.prepare(`
          SELECT kanji_page as kanjiPage, keisan_page as keisanPage,
                 other_tasks as otherTasks, tests, week_key as weekKey, active_days as activeDays
          FROM class_weekly_menu WHERE class_id = ? AND week_key = ? LIMIT 1
  ...
  `).bind(t.id,s).first();return e.json({ok:!0,reflection:a||null})});h.get("/api/student/dashboard",async e=>{const t=e.get("user");if(!t)return o(e,403,"forbidden");const s=J(),a=Ge(s),n=await e.env.DB.prepare(`
...
- Use encouraging, warm tone like a real teacher writing in a notebook (e.g. "1週間よくがんばったね！" "毎日続けられてえらい！")
- Return ONLY valid JSON, no other text: {"comments":["コメント1","コメント2",...]}
- The number of comments must exactly match the number of students listed
